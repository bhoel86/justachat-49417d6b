...
// Pad and center ASCII art lines to a fixed width (80 chars default)
// Formula: (totalWidth - textWidth) / 2 = leading spaces
// All lines padded to same length so columns align perfectly in fixed-width fonts
function padAsciiArt(lines: string[], totalWidth: number = 80): string[] {
  const maxLen = Math.max(...lines.map(l => l.length));
  return lines.map(line => {
    // First pad every line to the same width (right-pad with spaces)
...
// ASCII art banners for each room (raw - will be centered by padAsciiArt)
